

export var route = 
{
    "getCompetitions":"https://competition-ctf-uzvdtuvxfj.cn-hongkong.fcapp.run/competitions", //getCompetitions+'?page=0'

    "login":"https://auth-ctf-guopkzqcxo.cn-hongkong.fcapp.run/login",

    "register":"https://auth-ctf-guopkzqcxo.cn-hongkong.fcapp.run/register",

    "signupCompetition":"https://competition-ctf-uzvdtuvxfj.cn-hongkong.fcapp.run/competitions/{uuid}/attend",

    "users":"https://auth-ctf-guopkzqcxo.cn-hongkong.fcapp.run/users",
    
    "getCompetitionInfo":"https://competition-ctf-uzvdtuvxfj.cn-hongkong.fcapp.run/competitions/{uuid}",
    
    "getChallenges":"https://competition-ctf-uzvdtuvxfj.cn-hongkong.fcapp.run/competitions/{uuid}/problems",
    
    "getOneChallenge":"https://competition-ctf-uzvdtuvxfj.cn-hongkong.fcapp.run/competitions/{cuuid}/problems/{uuid}",

    "instance":"https://competition-ctf-uzvdtuvxfj.cn-hongkong.fcapp.run/competitions/{cuuid}/problems/{puuid}/backend",
    
    "addChallenge":"https://competition-ctf-uzvdtuvxfj.cn-hongkong.fcapp.run/competitions/{uuid}/problems",

    "newChallenge":"https://competition-ctf-uzvdtuvxfj.cn-hongkong.fcapp.run/competitions/{uuid}/problems",

    "newCompetition":"https://competition-ctf-uzvdtuvxfj.cn-hongkong.fcapp.run/competitions",

    "submitFlag":"https://competition-ctf-uzvdtuvxfj.cn-hongkong.fcapp.run/competitions/{cuuid}/problems/{puuid}/submit"

}




